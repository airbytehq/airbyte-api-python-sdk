# SourceEbayFinanceRefreshTokenEndpoint

## Example Usage

```python
from airbyte_api.models import SourceEbayFinanceRefreshTokenEndpoint

value = SourceEbayFinanceRefreshTokenEndpoint.HTTPS_API_SANDBOX_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN
```


## Values

| Name                                                  | Value                                                 |
| ----------------------------------------------------- | ----------------------------------------------------- |
| `HTTPS_API_SANDBOX_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN` | https://api.sandbox.ebay.com/identity/v1/oauth2/token |
| `HTTPS_API_EBAY_COM_IDENTITY_V1_OAUTH2_TOKEN`         | https://api.ebay.com/identity/v1/oauth2/token         |