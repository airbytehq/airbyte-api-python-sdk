...
class Workspaces:
    ...
    def delete_workspace_o_auth_credentials(self, request: api.DeleteWorkspaceOAuthCredentialsRequest) -> api.DeleteWorkspaceOAuthCredentialsResponse:
        ...
        """
        hook_ctx = HookContext(operation_id='deleteWorkspaceOAuthCredentials', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = utils.generate_url(base_url, '/workspaces/{workspaceId}/oauthCredentials/{actorType}/{name}', request)
        ...
        res = api.DeleteWorkspaceOAuthCredentialsResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res)
