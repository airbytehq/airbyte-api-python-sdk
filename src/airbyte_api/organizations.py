...
class Organizations:
    ...
    def delete_organization_o_auth_credentials(self, request: api.DeleteOrganizationOAuthCredentialsRequest) -> api.DeleteOrganizationOAuthCredentialsResponse:
        ...
        """
        hook_ctx = HookContext(operation_id='deleteOrganizationOAuthCredentials', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = utils.generate_url(base_url, '/organizations/{organizationId}/oauthCredentials/{actorType}/{name}', request)
        ...
        res = api.DeleteOrganizationOAuthCredentialsResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res)
